...
// events
event PausableControllerChanged(
    address previousPausableController,
    address newPausableController
);
 ...
 *
 */
abstract contract PausableController is
    OwnableUpgradeable,
    PausableUpgradeable
{
    ...
    modifier onlyOwnerOrPausableController() {
        require(
            msg.sender == pausableController || msg.sender == owner(),
            NotPausableControllerOrOwner(msg.sender)
        );
        _;
    }
    ...
    function changePausableController(
        address newPausableController
    ) public onlyOwner {
        require(
            newPausableController != address(0),
    ...
    function _changePausableController(address newPausableController) internal {
        ...
        emit PausableControllerChanged(
            previousPausableController,
            newPausableController
        );
    }
}
